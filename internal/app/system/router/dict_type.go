...
func (r *Router) BindDictTypeController(_ *gin.RouterGroup, auth *gin.RouterGroup) {
	apiController := system.NewDictTypeApi(dictType.New(bizctx.New()))
	protectedRouter := auth.Group("dictType")
	{
		...
		protectedRouter.GET("getDictData", grouter.HandlerFunc(apiController.GetDictData))
		protectedRouter.POST("getDictsData", grouter.HandlerFunc(apiController.GetDictsData))
	}
}
