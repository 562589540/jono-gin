...
func GenerateAndCacheLoginToken(ctx context.Context, userId uint, userName string) (string, time.Time, error) {
	duration := gbootstrap.Cfg.Jwt.TokenExpire * time.Minute
...
func GenerateAndCacheRefreshToken(ctx context.Context, userId uint, userName string) (string, error) {
	token, err := GenerateRefreshToken(userId, userName)
	if err == nil {
		err = ghub.RedisClient.Set(ctx, RefreshRedisTokenKey(userId), token, gbootstrap.Cfg.Jwt.RefreshTokenExpire*time.Hour)
	}
	return token, err
}
...
func ResolveGinToken(ctx *gin.Context, option ResolveGinTokenOption) (jwtCostClaims *JwtCostClaims, expireDuration time.Duration, err error) {
	var (
		rToken    string
		rTokenKey string
	...
	//解析token
	jwtCostClaims, err = ParseToken(tokenReq)
